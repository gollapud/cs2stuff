...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E>
  ...
  private E calculateTotalAtDepth(Position<E> position, int depth,  FoldFunction<E, E> f) {
    E e = f.initialValue();
    for (Position<E> child : children(position)) {
      if (depth(child) == depth) {
        e = f.combine(e, child.getElement());
      } else {
        e = f.combine(e, calculateTotalAtDepth(child, depth, f));
      }
    }
    return e;
  }
  ...
  using the given FoldFunction
   */

  public E foldAtDepthRecursive(int depth, FoldFunction<E, E> f) {
    E e = f.initialValue();
    ...
    if (depth > maxDepth) {
      ...
      e = calculateTotalAtDepth(position, depth, f);
    }
    return e;
  }
  ...
  using the given FoldFunction
   */
  public E foldAtDepthIterative(int depth, FoldFunction<E, E> f) {
    E e = f.initialValue();
    int maxDepth  = height(root());
    if (depth > maxDepth) {
      f.initialValue();
  ...
  /* Returns the List of elements for which the given Predicate returns true
   */
  public List<E> filterIterative(Predicate<E> p) {
    return null;
  }

  /* Returns the List of elements for which the given Predicate returns true
   */
  public List<E> filterRecursive(Predicate<E> p) {
    List<E> ll = new LinkedList<>();
    if (!this.isEmpty()) filterRecursiveHelper(p, ll, this.root());
    return ll;
  }

  private void filterRecursiveHelper(Predicate<E> p, List<E> results, Position<E> n) {
    if (p.check(n.getElement())) {
      results.add(n.getElement());
    }
    if (left(n) != null) {
        filterRecursiveHelper(p, results, left(n));
    }
    if (right(n) != null) {
      filterRecursiveHelper(p, results, right(n));
    }
  }
